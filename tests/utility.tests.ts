...
suite('Utility', () => {
	...
	suite('calculateTabSize', () => {
		...
		test('tab all formats, size 1', () => {
			const res = Utility.calculateTabSizes('${hex}\t1${signed}\t${unsigned}2\t${bits}\t${flags}', 1);
	...
	suite('numberFormattedBy', () => {
		...
		suite('formats', () => {
			...
			test('formats, size 1', async () => {
				const format = '${name},${hex},${signed},${unsigned},${bits},${flags}';
		...
		suite('tabs', () => {
			...
			test('special test 1', async () => {
				const format = "${b#:hex}h\t${b#:unsigned}u\t${b#:signed}i\t${b#:bits}b";
				const tabSizeArr = Utility.calculateTabSizes(format, 1);
			...
			test('special test 2', async () => {
				const format = "${b#:signed}i\t${b#:bits}b";
				const tabSizeArr = Utility.calculateTabSizes(format, 1);
