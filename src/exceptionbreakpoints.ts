 ...
 * ZX81 Debugger
 * 
 * File:			exceptionbreakpoints.ts
 * Description:		Enabling/disabling of the breakpoints used for assertions (ASSERT),
...
/**
 * The enabling/disabling of the breakpoints used for assertions (ASSERT),
...
export class ExceptionBreakpoints {
		...
		if (abps.length == 0) {
			...
			if (realEnable) {
				...
				for (const abp of abps) {
					result += Utility.getAddressString(abp.address);
					const labels = Labels.getLabelsForAddress(abp.address);
					if (labels.length > 0) {
	...
	protected async enableWPMEMs(enable: boolean): Promise<string> {
		...
		if (realEnable) {
			...
			for (const wp of wps) {
				result += Utility.getAddressString(wp.address);
				const labels = Labels.getLabelsForAddress(wp.address);
				if (labels.length > 0) {
					const labelsString = labels.join(', ');
					result += ' (' + labelsString + ')';
				}
		...
		if (enableMap.size == 0)
			...
			for (const [group, enabled] of enableMap) {
				...
				if (enabled) {
					...
					for (const lp of lps) {
						result += '\n    ' + Utility.getAddressString(lp.address);
						const labels = Labels.getLabelsForAddress(lp.address);
						if (labels.length > 0) {
							const labelsString = labels.join(', ');
							result += ' (' + labelsString + ')';
						}
