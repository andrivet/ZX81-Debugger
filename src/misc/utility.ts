...
export class Utility {
	...
	/**
	 * Returns a hex string from an address.
	 * "F7A4h"s.
	 * @param value The number to convert
	 * @returns E.g. "F7A4h"
	 */
	public static getAddressString(value: number): string {
		return this.getHexString(value & 0xFFFF, 4) + 'h';
	}
	 ...
	 * @param modulePrefix An optional prefix to use for each label.
	 * @param lastLabel An optional last label to use for local labels.
	...
	*/
	 ...
	 * @returns the URI of an asset.
	 */
	public static getZX81ImageSrc(value: number): vscode.Uri {
		return Utility.getAssetSrc('chars/ZX81-0x' + this.getHexString(value, 2) + '.png');
	}
	...
	public static ensureDirExist(directory: string) {
		const absFPath = Utility.getAbsFilePath(directory);
		if (fs.existsSync(absFPath)) return;
		fs.mkdirSync(absFPath, {recursive: true});
	}
	...
	public static getRelTmpFilePath(fileName: string): string {
		...
		this.ensureDirExist(Settings.launch.tmpDir);
		return relFilePath;
	}
	 ...
	 */
	public static convertHexNumber(hexNumber: number | string | undefined): number | undefined {
