 ...
 * Description:		The Emulator Debug Adapter.
...
import {DisassemblyVar, ImmediateMemoryValue, MemDumpVar, RegistersMainVar, RegistersSecondaryVar, ShallowVar, StackVar, StructVar, SystemVariablesVar} from './variables/shallowvar';
...
class StackFrameAddr extends StackFrame {
	public address: number;	// The associated address
}
...
export class DebugSessionClass extends DebugSession {
	...
	/// Is true if a zx81debugger debug session is running.
			...
			Remote.once('initialized', async (text) => {
				...
				try {
					...
					if (Remote instanceof ZSimRemote) {
						// Show the ZX81 Simulator view
						const zsim = Remote;
						const view = new Zx81View(zsim);
						view.update(false);
					}
				}
				catch(e) {
	...
	protected async setBreakPointsRequest(response: DebugProtocol.SetBreakpointsResponse, args: DebugProtocol.SetBreakpointsArguments): Promise<void> {
		...
		const vscodeBreakpoints = givenBps.map(gbp => {
			...
			if (foundCbp && foundCbp.address >= 0) {
				...
				const addrString = Utility.getAddressString(foundCbp.address);
				// Add hover text
				let txt = addrString;
	...
	protected async stackTraceRequest(response: DebugProtocol.StackTraceResponse, _args: DebugProtocol.StackTraceArguments): Promise<void> {
		...
		const [sfrs, callstackAddresses] = this.stackFramesForCallStack(callStack);
		...
		try {
			...
			const disasmUpdated = await Disassembly.setNewAddresses(callstackAddresses);
	...
	protected stackFramesForCallStack(callStack: RefList<CallStackFrame>): [StackFrameAddr[], number[]] {
		...
		for (let index = frameCount - 1; index >= 0; index--) {
			...
			let src;
			if (file.size > 0) { // This is required, otherwise a label with a bp only in rev-eng would lead to a non-disaassembled instruction at that line.
...
//		vscode.languages.setTextDocumentLanguage(disasmTextDoc, 'disassembly');
		...
		vscode.commands.executeCommand('setContext', 'zx81debugger.disassembler.disasmPath', [absFilePath]);
	...
	protected async endStepInfo(): Promise<void> {
		...
		if (tStates) {
			...
			if (cpuFreq) {
				...
				// CPU clock
				let clockStr = (cpuFreq * 1E-6).toPrecision(3);
			...
			case 'hover':
				...
				try {
					...
					if (Z80RegistersClass.isRegister(expression)) {
						...
						// First check for module name and local label prefix.
						const pcAddr = Remote.getPC();
						const entry = Labels.getFileAndLineForAddress(pcAddr);
	...
	protected async evaluateLabelExpression(expression: string): Promise<ExpressionVariable> {
		...
		const description = Utility.getAddressString(labelValue64k);
...
`;
	...
	protected async evalAddress(tokens: Array<string>): Promise<string> {
		...
		if (tokens.length === 1) {
			...
			const e = Labels.getFileAndLineForAddress(addr64k);
			txt += 'File: ';
			if (e.fileName) {
			...
			for (let addr = 0x0000; addr < 0x10000; addr++) {
				const e = Labels.getFileAndLineForAddress(addr);
				if (e.fileName) {
					count++;
	...
	*/
	 ...
	 * @returns [fromAddr, toAddr] Start and end address.
	...
	protected checkFileLinesPagedIn(filename: string, fromLineNr: number, toLineNr: number): Array<number> {//</number> {fromAddr: number, toAddr: number} {
		...
		while (address < 0xFFFF) {
			const addr = (address + 1) | upperAddr;
			const fileLine = Remote.getFileAndLineForAddress(addr);
	...
	public async disassemblyAtCursor(type: 'code' | 'data' | 'string', filename: string, fromLineNr: number, toLineNr: number): Promise<void> {
		...
		let toAddr: number;
		try {
			[fromAddr, toAddr] = this.checkFileLinesPagedIn(filename, fromLineNr, toLineNr);
	...
	public async analyzeAtCursor(type: 'disassembly' | 'flowChart' | 'callGraph', arr: Array<{filename: string, fromLine: number, toLine: number}>): Promise<void> {
		...
		try {
			...
			let titles: string[] = [];
			for (const addr of startAddrs) {
				// Get label
				const labels = Labels.getLabelsForAddress(addr);
				let name;
				...
				if (!name)
					name = Utility.getHexString(addr & 0xFFFF, 4) + 'h';
				// Add to title
				titles.push(name);
			}
	...
	protected installHtmlClickHandler(view: HtmlView) {
		...
		view.on('click', async message => {
			...
			if (k < 0)
				return;
			const addrString = addressesString.substring(k + 1);	// Skip #
			// Check if empty
			if (addrString === '') {
			...
			// Separate addresses
			const addresses = addrString.split(';');
			...
			for (const addressString of addresses) {
				...
				const addr = parseInt(addressString, 16);	// Convert hex, e.g. ("C000h")
				const entry = Remote.getFileAndLineForAddress(addr);
	 ...
	 * Only the list files are reloaded, not the launch.json, nor the binary.
	 */
