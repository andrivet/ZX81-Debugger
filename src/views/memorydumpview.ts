 ...
 * ZX81 Debugger
 * 
 * File:			memorydumpview.ts
 * Description:		A Webview that shows a memory dump.
            ...
            switch (message.command) {
				...
				case 'memoryChanged':
				{
					...
					for(const addrVal of changedAddressValues) {
						...
						for(const obj of objs) {
							obj.classList.remove("addressChanged");
						}
					...
					for(const addrVal of changedAddressValues) {
						...
						for(const obj of objs) {
							...
							obj.classList.add("addressChanged");
						}
						// Get Zx81 chars for address
						const CharObjs = getCharObjsForAddress(address);
						for(const obj of CharObjs) {
							obj.firstChild.src = addrVal[2];
				...
				case 'foundAddresses':
				{
					...
					// Char
					for(const obj of foundAddressesCharObjs) {
					...
					// Char
					foundAddressesCharObjs = [];
					for(const address of foundAddresses) {
						for(let i=0; i<selectedLength; i++) {
							const objs = getCharObjsForAddress(address+i);
							for(const obj of objs) {
								foundAddressesCharObjs.push(obj);
								obj.classList.add("foundAddressChar");
							}
						}
					}
...
`;
	...
	protected createHtmlTable(metaBlock: MetaBlock): string {
		...
		let clmStart = address % this.columns;
		table += '<tr>\n<th>Address:</th> <th></th>';
		...
		for (let k = 0; k < len; k++) {
			...
			char += '<span address="' + address + '" onmouseover="mouseOverValue(this)">';
			char += '<img src="' + this.vscodePanel.webview.asWebviewUri(Utility.getZX81ImageSrc(value)) + '">';
			char += '</span>';
		...
		const html = util.format(format, this.columns, table);
		return html;
	}
