 ...
 * ZX81 Debugger
 * 
 * File:			dzrpbufferedremote.ts
 * Description:		Conversion of SendDzrpCmd... functions as buffer and parsing of received messages
...
import {AlternateCommand, DZRP, DZRP_PROGRAM_NAME} from '../dzrp/dzrpremote';
	...
	protected receivedMsg(data: Buffer) {
		...
		if (recSeqno == 0) {
			...
			if (this.funcContinueResolve) {
				...
				// Get data
				const type = data[2];
				let addr = Utility.getWord(data, 3);
				const breakAddressBank = data[5];
				addr += breakAddressBank << 16;
				// Get reason string
				...
				// Handle the break.
				continueHandler({reasonNumber: type, addr, reasonString});
			}
		}
		else {
			// Stop timeout
