 ...
 * kinds of breakpoints but in ZX81 Debugger they are differentiated.
 *
 */
export class RemoteBase extends EventEmitter {
	...
	/// Intialized at 0x8000 for ZX81 16K
	public readonly topOfStack: number = 0x8000;

	public readonly loadingAddress: number = 0x4009;

	public readonly startAddress: number = 0x4082;
	...
	public async compileCode(): Promise<CompilationError[]> {
		...
		if(Settings.launch.binary) return [];
	...
	/**
	 * Loads the executable to debug.
	 * Do not override.
	 * Override
	 */
	public async load(): Promise<void> {
		const loadPath = Settings.launch.binary;
		if (loadPath)
	...
	public async setLaunchExecAddress(): Promise<void> {
		// Set PC
		await this.setRegisterValue("PC", this.startAddress);
	}
	...
	protected async getStackEntryType(stackEntryValue: string): Promise<{name: string, callerAddr: number} | undefined> {
		...
		// Found: get label
		let labelCalledAddrArr = Labels.getLabelsForAddress(calledAddr);
	...
	*/
	...
	public async setBreakpoints(path: string, givenBps: Array<RemoteBreakpoint>): Promise<Array<RemoteBreakpoint>> {
			...
			givenBps.forEach(bp => {
				...
				if (addr >= 0) {
					...
					if (path.valueOf() == file.fileName.valueOf()) {
						...
						error = "You cannot set a breakpoint here because the address (" + Utility.getHexString(addr & 0xFFFF, 4) + "h) is bound to a different file. Please try to set the breakpoint in: " + file.fileName;
					}
				}
				else {
					// Additional info
		...
		catch (e) { // Catch is here just for debugging.
			throw e;
		}
	}
	 ...
	 * ZX81 Debugger will set a breakpoint there and execute a 'continue' to simulate
	 * a 'step'.
	 * But for branching or conditional branching instructions this is different.
	 * ZX81 Debugger will then use up to 2 breakpoints to catch up after the instruction is executed.
	...
	protected async calcStepBp(stepOver: boolean): Promise<[Opcode, number, number?]> {
		...
		while (true) {
			skip = skipAddresses.get(bpAddr1);
			if (!skip)
				break;
	 ...
	 * @returns An array with addresses.
	 */
