 ...
 * ZX81 Debugger
 * 
 * File:			zesaruxremote.ts
 * Description:		Representation of the ZEsarUX emulator.
...
export class ZesaruxRemote extends RemoteBase {
	...
	public async continue(): Promise<string> {
		return new Promise<string>(resolve => {
			...
			zSocket.sendInterruptableRunCmd(async text => {
				...
				const pc = this.getPC();
				const abps = this.assertionBreakpoints.filter(abp => abp.address == pc);
	...
	/**
	 * Loads compiled binary.
	 * @param path The (absolute) path to the file.
	 */
	public async loadBin(path: string): Promise<void> {
		await zSocket.sendAwait('smartload "' + Settings.launch.binary + '"');	// Note: this also changes cpu to tbblue
	}
}
