 ...
 * 
 * File:			renderhtml.ts
 * Description:		Render disassembly text
...
export class RenderHtml extends RenderText {
	...
	<style>
		* {
		...
		body.vscode-light {
			--zx81debugger-fg-color-emphasize-label: #001080;
			--zx81debugger-bg-color-emphasize-startlabel: lightblue;
			--zx81debugger-fg-color-emphasize-comment: #008000;
			--zx81debugger-fg-color-emphasize-bytes: ##098658;
			--zx81debugger-fg-color-emphasize-instruction: #0000FF;
			--zx81debugger-fg-color-emphasize-data: #098658
		}

		body.vscode-dark {
			--zx81debugger-fg-color-emphasize-label: #9CDCFE;
			--zx81debugger-bg-color-emphasize-startlabel: navy;
			--zx81debugger-fg-color-emphasize-comment: #6A9955;
			--zx81debugger-fg-color-emphasize-bytes: #B5CEA8;
			--zx81debugger-fg-color-emphasize-instruction: #569CD6;
			--zx81debugger-fg-color-emphasize-data: #B5CEA8
		}
		...
		.startlabel {
			background:var(--zx81debugger-bg-color-emphasize-startlabel);
			font-weight:bold;
		}
		.label {
			color:var(--zx81debugger-fg-color-emphasize-label);
		}
		.comment {
			color:var(--zx81debugger-fg-color-emphasize-comment);
			font-weight:bold;
		}
		.bytes {
			color:var(--zx81debugger-fg-color-emphasize-bytes);
		}
		.instruction {
			color:var(--zx81debugger-fg-color-emphasize-instruction);
		}
		.data {
			color:var(--zx81debugger-fg-color-emphasize-data);
		}
    </style>
	...
	protected addReferences(text: string, addr64k: number): string {
		const href = 'href="#' + this.disasm.funcFormatAddress(addr64k) + '"';
	...
	protected formatAddressPlusText(addr64k: number, bytes: Uint8Array, text: string, comment?: string): string {
		...
		const addrString = this.disasm.funcFormatAddress(addr64k).padEnd(this.clmnsAddress - 1);
		let bytesString = '';
		bytes.forEach(value =>
